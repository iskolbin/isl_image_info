...
static unsigned char islii__fread_u8( islii_file f, islii_fread read ) {
	unsigned char a;
	read( &a, 1, 1, f );
	return a;
}
...
static islii_result fread_png( islii_file f, islii_fread read ) {
	...
	if ( third == 78 ) {
		fskip( f, read, 13 );
		result.width = islii__fread_u32_big( f, read );
		result.height = islii__fread_u32_big( f, read );
		result.format = ISLII_PNG;
		result.bpp = (unsigned int) islii__fread_u8( f, read );
		switch ( islii__fread_u8( f, read )) {
...
static islii_result fread_bmp( islii_file f, islii_fread read ) {
	...
	fskip( f, read, 16 );
	result.width = islii__fread_u32_little( f, read );
	result.height = islii__fread_u32_little( f, read );
	islii__fread_u16_little( f, read );
	result.bpp = islii__fread_u16_little( f, read );
	switch ( result.bpp ) {
...
static islii_result fread_gif( islii_file f, islii_fread read ) {
	islii_result result = { ISLII_ERROR_OK, ISLII_UNKNOWN, ISLII_UNKNOWN_COLORTYPE, 0, 0, 0 };
	if ( islii__fread_u8( f, read ) == 'F' ) {
		fskip( f, read, 3 );
		result.width = islii__fread_u16_little( f, read );
		result.height = islii__fread_u16_little( f, read );
...
static islii_result fread_jpeg( islii_file f, islii_fread read ) {
	...
	for(;;) switch (islii__fread_u16_big( f, read )) {
		...
		case 0xffef: case 0xfffe: case 0xffdb: case 0xffc4: case 0xffda: 
			fskip( f, read, ((long int) islii__fread_u16_big( f, read )) - 2 );
			break;

		case 0xffc0: case 0xffc2:
			fskip( f, read, 2 );
			result.bpp = (unsigned int) islii__fread_u8( f, read );
			result.width = islii__fread_u16_big( f, read );
			result.height = islii__fread_u16_big( f, read );
			switch ( islii__fread_u8( f, read )) {
...
islii_result islii_info( islii_file f, islii_fread read ) {
	if ( !f ) {
		...
		switch ( islii__fread_u16_big( f, read )) {
